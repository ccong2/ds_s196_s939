...
Notice that this shapefile has population by neighborhood that is ready to use, which is very nice, otherwise we will have to aggregate the census population ourselves.
...
bbox = c(42.22791,-71.19124,42.39698,-70.80449)
```
...
Following the initial `opq(bbox)` call, we will build queries by adding one or more features, which are specified in terms of *key-value pairs*. [Here](https://wiki.openstreetmap.org/wiki/Map_Features) is a complete list of key-value pairs on OSM, in which values can be understood as specific types of places, and keys are their categories.
