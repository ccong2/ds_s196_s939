...
View the dataset in your data viewer. While browsing its variables, there might be here are something we are interested to know:
...
1.  In Massachusetts, how many eligible census tracts are designated as Opportunity Zones, and how many are not designated? What are the average poverty rates of the two groups?

2.  When you have the result of Q1 (the summary for all of Massachusetts), what are the corresponding situations by county in Massachusetts?
...
The following code creates a **bar chart**. You will see our familiar `group_by` + `summarise` process used to calculate the average median house income by county in Massachusetts. This summarized table is then piped to `ggplot()` for visualization, and the `geom` function used here is `geom_col`.
...
ozs_ma |> 
  ...
  summarise(
    Tracts = n(),
    Income = mean(medhhincome, na.rm=TRUE)) |> 
  ggplot(aes(x = county, y = Income, fill = DesignatedOZ)) +
  geom_col() 
```
...
There are a few differences, which have enhanced the overall quality. How can you modify your code above to replicate the bar chart in this image? In a new code chunk, please copy and paste your last bar chart code, and try your best to address the following questions.
...
Please submit **a** **Rendered HTML file** that shows your work and responses for each of the three Exercises included in this lab. Make sure you have included `embed-resources: true` in your YAML header, this is to help preserve all your pictures in your HTML.
